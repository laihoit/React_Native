...
class HistoryPostSale extends Component {
    constructor(props){
        super(props);
        items=[]
        this.state = {
    ...
    componentDidMount(){
        that = this;
        var query = firebaseApp.database().ref('PostSale').orderByChild('User').equalTo(this.props.mystate);
...
export default connect(mapStateToProps)(HistoryPostSale);
